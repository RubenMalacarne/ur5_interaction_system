...
namespace cr::scene_management
{
        ...
        : Node("planning_scene_modifier", options)
    {
        ...
        // Service for modifying the collision matrix
        ...
        // Service for attaching/detaching objects
        ...
        std::shared_ptr<cr_interfaces::srv::AllowCollision::Response> response)
    {
        ...
        if (!psm || !psm->getPlanningScene())
        {
            RCLCPP_ERROR(get_logger(), "[allowCollision] PlanningSceneMonitor or PlanningScene unavailable.");
        ...
        {
            RCLCPP_ERROR(get_logger(), "[allowCollision] planning_scene_pub_ not initialized.");
            response->success = false;
            return;
        }

        planning_scene_pub_->publish(scene_msg);
        RCLCPP_INFO(get_logger(), "[allowCollision] Updated ACM for object %d. Collision %s.",
                    request->object_id,
                    request->is_allowed ? "ALLOWED" : "DENIED");
